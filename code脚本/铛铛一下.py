...
2026/07/07  V1.6    适配YYB-Go协议，优化青龙原生推送
"""
import random
import time
import requests
...
NOTIFY = os.getenv("LY_NOTIFY", "").lower() == "true"  # 是否推送日志，设置true开启
    ...
    def check_env(self):
        ...
        try:
            ...
            for soy_wxid_data in soy_wxid_datas:
                wxid = soy_wxid_data.strip()
                if "=" in wxid:
                    wxid = wxid.split("=")[1].strip()
                if wxid:
                    yield wxid
        except Exception as e:
        ...
        try:
                ...
                if code:
                    ...
                    if login_result:
                        ...
                        while add_lottery_count_result:
                            ...
                            add_lottery_count_result = self.add_lottery_count(session)
                        # 获取提现相关数据
                        res = self.get_withdrawal_trade_list(session)
                        if res:
                            balance, withdrawal_trade_list = res
            ...
            if NOTIFY:
                ...
                try:
                    ...
                    if not os.path.exists("notify.py"):
                        try:
                            ...
                            self.log(f"[推送] 下载notify模块失败: {str(e)}", level="error")
                ...
                if notify_module:
                    ...
                    self.log("[推送] 未找到可用的notify模块，跳过推送", level="warning")
