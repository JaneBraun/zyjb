...
"""
旧衣小二旧衣服回收小程序签到脚本（适配器版）

功能：
  1. 统一微信协议适配器获取code（支持YYB-Go/养鸡场/牛子等全协议）
  2. 使用 code 换取 token
  3. 每日签到
  4. 查询积分余额
  5. 查询积分明细
...
环境变量：
  ...
  soy_codeurl_data    微信授权服务地址
  soy_codetoken_data  微信授权token（部分协议需要）
  PLUSPLUS_TOKEN      PushPlus token，可选
  PROXY_API           品赞代理提取 API，可选
  PROXY_TYPE          http / socks5，默认 http
  LY_NOTIFY           填true开启青龙原生推送
...
更新日志:
2025/xx/xx  V1.0    初始化
2026/07/07  V2.0    适配统一微信协议适配器（支持YYB-Go），新增青龙原生推送
"""
...
MULTI_ACCOUNT_SPLIT = ["\n", "@"]  # 多账号分隔符
NOTIFY = os.getenv("LY_NOTIFY", "").lower() == "true"  # 青龙推送开关
...
if not os.path.exists(wechat_adapter_path):
    try:
        ...
        print(f"下载微信协议适配器失败（网络问题），请手动放置 wechatCodeAdapter.py")
        exit(1)
    except Exception as e:
        print(f"下载微信协议适配器失败：{e}")
        exit(1)
...
def run_account(index: int, total: int, wx_id: str, adapter: WechatCodeAdapter) -> Dict[str, Any]:
    ...
    log_account_header(index, total, mask(wx_id))

    proxies, proxy_ip = get_valid_proxy(mask(wx_id))
    ...
    # 使用统一适配器获取code，自动适配YYB-Go
    code = adapter.get_code(wx_id)
    if not code:
        result["error"] = "获取 code 失败"
        return result

    token, raw_login = login_by_code(mask(wx_id), code, proxies)
    if not token:
    ...
    try:
        person_resp = api_post_form(mask(wx_id), PERSON_INFO_URL, token, proxies, {"token": token})
        ...
        sign_resp = api_post_form(mask(wx_id), SIGN_URL, token, proxies, {"token": token})
        ...
        person_after = api_post_form(mask(wx_id), PERSON_INFO_URL, token, proxies, {"token": token})
        ...
        score_list_resp = api_post_form(mask(wx_id), SCORE_LIST_URL, token, proxies, {
            "page": 1,
            "limit": 5,
            "token": token
        })
...
def main() -> None:
    ...
    # 原有PushPlus推送保留
    #send_pushplus("♻️ 旧衣小二签到任务完成", notify_content)
    # 新增青龙原生推送
    if NOTIFY:
        send_qinglong_notify("♻️ 旧衣小二签到任务完成", notify_content)
